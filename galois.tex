...
\end{align*}
It is a finite group of order $1,451,520$. The symplectic group over $\mathbb{F}_2$ is defined in \Cref{sec:quad-forms-f2}, while the connection to the Weyl group passes through the theory of Lie groups. Some references for this connection:
\begin{itemize}
...
\begin{remark}[The Bring family] Consider the family of polynomials $p_a(z) = z^5 + az + 1$ for $a\in \C$. This is a linear subspace of degree $1$ in the moduli of monic quintics.
\begin{enumerate}
...
% \begin{proposition}[{\cite[p.189]{Dolgachev}}]
% Two quadratic forms $q,q'$ on $V\in \Vect_k$ have the same polarization if  $q-q' = \ell$ for some $\ell \colon V\to k$ with
% \begin{itemize}
%     \item $\ell(v+w) = \ell(v) + \ell(w)$
%     \item $\ell(av) = a^2 \ell(v)$, for any $v\in V$, $a\in k$
% \end{itemize}
% \end{proposition}
% \begin{proof} For the forwards direction, we set
% \begin{align*}
%     q(v+w) - q(v) - q(w) &= q'(v+w) - q'(v) - q'(w),
% \end{align*}
% and rearrange to get additivity of $\ell = q-q'$, then it is clear $\ell(av) = a^2 \ell(v)$. For the backwards direction, we take $q'$ and any such $\ell$, and show that $q:= q' + \ell$ is quadratic with the same polarization as $q'$.
% \end{proof}
...
\begin{proposition}[{\cite{Beauville_delPezzo_lattices}}] Let $L$ be a del Pezzo lattice of rank $\ge 4$.
\begin{enumerate}
