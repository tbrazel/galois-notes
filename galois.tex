...
Jordan noted (in modern language) that any monodromy of the flexes over the moduli of cubics will preserve the Hesse arrangement, in particular the resulting permutation of the nine flexes will preserve the colinearity properties they satisfy \cite[Theorem~425]{Jordan}. In particular the monodromy will be a subgroup of the graph automorphisms of the graph in \autoref{fig: A2 F3}. This automorphism group can be identified with the affine general linear group $\ASL(2,\F_3)$, given by the semidirect product of the translations and rotations $\ASL(2,\F_3) \cong \F_3^2 \rtimes \SL(2,\F_3)$. That is:
\begin{align*}
    \Mon(\text{flexes on cubics}) \le \ASL(2,\F_3) \le \Sigma_9.
\end{align*}
We will show this first inclusion is an equality, following \cite{Harris-Galois}.
