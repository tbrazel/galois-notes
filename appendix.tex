...
Let $\mathbb P^2_{x,y,z}$ be the projective plane. A \textit{circle} is a plane conic curve that satisfies any of the following equivalent characterizations:
\begin{itemize}
    \item Having affine equation $(x-a)^2+(y-b)^2 = r^2$ or homogeneous equation $(x-az)^2+(y-bz)^2 = r^2z^2$ for $a,b,r\in \mathbb C$;
...
where $\mathcal{F}$ is a skyscraper sheaf supported at the singular points. By the long exact sequence of sheaf cohomology, we have \begin{gather}
    0\to k^\times\to H^0(\pi_*\mathcal{O}_{\widetilde C}^\times)\to H^0(\mathcal{F}) \\
    \to \operatorname{Pic}(C) \to \operatorname{Pic}(\widetilde{C})\to H^1(\mathcal{F}) = 0
\end{gather}
where $H^1(\mathcal{F}) = 0$ since $\mathcal{F}$ is flasque. Let $\Gamma:=H^0(\mathcal{F})/\operatorname{im}H^0(\pi_*\mathcal{O}_{\widetilde C}^\times)$. There is a short exact sequence $$0\to \Gamma \to J(C)\xrightarrow{\pi^*} J(\widetilde{C})\to 0,$$
...
The above linear functional is called the \textbf{linear part of the theta-form}, denoted $l: \Gamma_2 \to \mathbb Z/2$.
\end{definition}
...
The counts of even and odd $\theta$-characteristics on $C$ now depend on whether $l$ is identically zero. Recall that $\widetilde{g}$ is the sum of the geometric genera of the irreducible components of $\widetilde{C}$ and $k$ is the rank of $\Gamma_2$ as a $\mathbb{F}_2$-vector space.
\begin{proposition}[\HarrisThetaCite{2a}, Cor 2.7]
If the linear part $l \not\equiv 0$, then
\[ |S^+| = |S^-| = \frac{1}{2}|S(C)| = 2^{2\widetilde{g} + k - 1}. \]
\end{proposition}
If the linear part $l \equiv 0$, then the values $|S^+|$ and $|S^-|$ depend on another invariant $q(C) = \sum_p \epsilon(p)(p) \pmod 2$, where $\epsilon(p)(p)$ is a local invariant of the singularity $p$ related to the adjoint divisor.
\begin{proposition}[\HarrisThetaCite{2a}, Cor 2.8] \label{prop: count theta}
If the linear part $l \equiv 0$, then
\begin{itemize}
    \item When $q(C)=0$, then $|S^+| = 2^{\widetilde{g}+k-1}(2^{\widetilde{g}}+1)$ and $|S^-| = 2^{\widetilde{g}+k-1}(2^{\widetilde{g}}-1)$.
    \item When $q(C)=1$, then $|S^+| = 2^{\widetilde{g}+k-1}(2^{\widetilde{g}}-1)$ and $|S^-| = 2^{\widetilde{g}+k-1}(2^{\widetilde{g}}+1)$.
\end{itemize}
\end{proposition}
...
\paragraph{From Three Circles to a Singular Genus 4 Curve $C$.} Let $C_1, C_2, C_3$ be three general circles in the plane. The union $C_\text{sum} = C_1 \cup C_2 \cup C_3$ is a degree 6 plane curve. It has triple points at $\circ_+, \circ_-$ and ordinary double points at the $3 \times 2 = 6$ pairwise intersection points of $C_1, C_2, C_3$. The partial normalization $\widetilde{C}_\text{sum}\to C_\text{sum}$ at $\circ_+$ and $\circ_-$ is a singular curve in $\mathrm{Bl}_{\circ_+,\circ_-}\mathbb P^2$ with $6$ ordinary nodes. We can use the degree-genus formula for $\mathbb P^2$ to calculate $$g(\widetilde{C}_\text{sum}) = \frac{(6-1)(6-2)}{2} - 6 = 4.$$ If we blow down the line $\overline{\cp\cm}$, then $\mathrm{Bl}_{\circ_+,\circ_-}\mathbb P^2$ is mapped to $\mathbb P^1\times \mathbb P^1$, which can be embedded in $\mathbb P^3$ as a quadric surface $Q$. Let $\cpm$ denote the image of the line $\overline{\cp\cm}$ and let $\hat{C}_\text{sum}$ be the image of $\widetilde{C}_\text{sum}$. Then, $\Hat{C}_\text{sum}$ is a singular genus $4$ curve embedded in $\mathbb P^3$ as a type $(2,3)$-complete intersection, so it is a canonical curve. As a result, the hyperplane sections of $Q$ are proper transforms of circles in $\mathbb P^2$, and the original plane $\mathbb P^2$ is related to $Q$ by projecting from the point $\cpm$. 
...
\[ |S^-| = 2^{2\widetilde{g} + k - 1} = 2^{2(0) + 4 - 1} = 2^{0 + 3} = 2^3 = 8 \]
...
\section{Real solutions to enumerative problem correspond to fixed points of involutions}

\begin{proposition}
    Let $\pi: Y\to X$ be a generically finite morphism of irreducible varieties of degree $d$ and relative dimension $0$. Let $U\subseteq X$ be an open set over which $V:=\pi^{-1}(U)\to U$ is finite étale. Assume $U(\mathbb R)\neq \varnothing$ and fix $p\in U(\mathbb R)$. Let $M$ be the monodromy group of $\pi$ acting on the fibers $F_p(\mathbb C) := \pi^{-1}(p)(\mathbb C)$. For a path $\gamma$ from $p$ to $p^\prime$, let $D_\gamma: F_p \to F_{p^\prime}$ be the deck transformation induced by $\gamma$. Then, 

    \begin{enumerate}
        \item Complex conjugation $c: Y(\mathbb C) \to Y(\mathbb C)$ restricts to an involution $c_p:F_p(\mathbb C) \to F_p(\mathbb C)$ on the fibers.
        \item The real fiber $F_p(\mathbb R)$ is the fixed locus of the involution $c_p$. 
        \item If $p$ and $p^\prime \in U(\mathbb R)$ lie in the same connected component, then \[c_{p^\prime} = D_\gamma c_{p} D_\gamma^{-1}.\] In particular, the number of real solutions $\# \pi^{-1}(p)(\mathbb R)$ depends only on the conjugacy class of $c \in M$.
    \end{enumerate}
\end{proposition}

\begin{proof}
    (1) Let $C_X, C_Y$ be the antiholomorphic involutions on $X(\mathbb C), Y(\mathbb C)$ induced by complex conjugation. We have $\pi\circ C_Y = C_X\circ \pi$. Since any real point $p\in U(\mathbb{R})$ is fixed by $C_X$, we have 
    \[C_Y(F_p) = C_Y(\pi^{-1}(p)(\mathbb C)) = \pi^{-1}(C_X(p)) = \pi^{-1}(p) = F_p,\]
    so $C_Y$ preserves the fiber $F_p$. In particular, the restriction $c_p := C_Y|_{F_p}$ is an involution on $F_p$. 

    (2) A point $y\in F_p(\mathbb C)$ is real if and only if $C_Y(y) = y$.

    (3) Follows from the fact that deck transformations commute with complex conjugation, i.e. $D_\gamma\circ c_p = c_{p^\prime} \circ D_\gamma$.
\end{proof}

\begin{table}[h] \centering \begin{tabular}{ll l}
\toprule \textbf{del Pezzo} & \textbf{Weyl group} & \textbf{Fixed points} \\ \midrule dP7 &
A2 & [1, 3] \\ dP5 & A4 & [2, 4, 10] \\ dP4 & D5 & [0, 4, 8, 16] \\ dP3 & E6 &
[3, 7, 15, 27] \\ dP2 & E7 & [0, 8, 16, 32, 56] \\ \bottomrule \end{tabular}
\caption{Fixed points of involutions in Weyl groups} \label{tab:fixed-point-count} \end{table}
